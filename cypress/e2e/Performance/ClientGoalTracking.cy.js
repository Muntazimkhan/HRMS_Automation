...
describe('Client Goal Tracking', () => {
    beforeEach(() => {
        cy.login(Cypress.env('VALID_EMAIL'), Cypress.env('VALID_PASSWORD'));
        cy.viewport(1440, 900);
    })

    it('Check the Client Goal Tracking functionality', () => {
        cy.get('a.dash-link').contains('Performance').should('be.visible').click();
        cy.contains('a', 'Client Goal Tracking').should('be.visible').click();

        //Create a new goal tracking
        cy.get('.ti.ti-plus').click();
        cy.get('.choices__inner').eq(0).click({ force: true });
        cy.get('.choices__list').contains('Attache').click({ force: true });
